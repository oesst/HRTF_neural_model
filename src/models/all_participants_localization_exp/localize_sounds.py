...
def main(model_name='localize_all_participant', exp_name='all_participants_default', azimuth=12, snr=0.0, freq_bands=24, max_freq=20000, elevations=25, mean_subtracted_map=True, ear='ipsi', normalization_type='sum_1', sigma_smoothing=0, sigma_gauss_norm=1, clean=False, steady_state=False):
    ...
    if not clean and exp_path.exists() and exp_file.is_file():
        ...
        # store responses
        r_ipsi_all = np.zeros((len(participant_numbers), len(SOUND_FILES), len(elevations), freq_bands))
        q_ele_all = np.zeros((len(participant_numbers), len(SOUND_FILES), len(elevations), len(elevations)))
        localization_results_binaural = np.zeros((len(participant_numbers), len(SOUND_FILES), len(elevations), 3))
        localization_results_monaural = np.zeros((len(participant_numbers), len(SOUND_FILES), len(elevations), 3))

        for i_par, participant_number in enumerate(participant_numbers):
            logger.info('Localizing sound for participant #'+str(participant_number))

            # create unique experiment name
            weights_name_str = hp.create_exp_name([exp_name, normalization_type, sigma_smoothing, sigma_gauss_norm, mean_subtracted_map, time_window, int(
                snr * 100), freq_bands, max_freq, participant_number, (azimuth - 12) * 10, normalize, len(elevations), ear])

            weights_file = exp_path / weights_name_str
            ...
            exp_file_weights = Path(weights_file.as_posix() + '_weights')
            ...
            for sound, _ in enumerate(SOUND_FILES):
                for i_ele, ele in enumerate(elevations):
                    ...
                    localization_results_monaural[i_par, sound, i_ele, 0] = ele
                    localization_results_monaural[i_par, sound, i_ele, 1] = q_ele[-1, :].argmax()

                    # PRIOR
                    q_ele, r_ipsi, w, w_sounds_i, w_sounds_c = net.run(in_i, in_c, ele, sound, train=False, prior_info=True)
                    # # localize and save results
                    localization_results_monaural[i_par, sound, i_ele, 2] = q_ele[-1, :].argmax()
            ...
            for sound, _ in enumerate(SOUND_FILES):
                for i_ele, ele in enumerate(elevations):
                    ...
                    q_ele, r_ipsi, w, w_sounds_i, w_sounds_c = net.run(in_i, in_c, ele, sound, train=False, prior_info=False)
                    # localize and save results
                    localization_results_binaural[i_par, sound, i_ele, 0] = ele
                    localization_results_binaural[i_par, sound, i_ele, 1] = q_ele[-1, :].argmax()

                    # PRIOR
                    q_ele, r_ipsi, w, w_sounds_i, w_sounds_c = net.run(in_i, in_c, ele, sound, train=False, prior_info=True)
                    # localize and save results
                    localization_results_binaural[i_par, sound, i_ele, 2] = q_ele[-1, :].argmax()
